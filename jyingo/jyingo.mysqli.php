 ...
 */
 ...
 class cls_fetch_i  implements Serializable {
  ...
  var $count;
    ...
    if ($this->dbg1_res && !$this->ids)
    {
	    ...
	    @mysqli_data_seek($this->dbg1_res, 0);
	    while ($arr =  @mysqli_fetch_assoc($this->dbg1_res))
	    {
	    	$ids[] = $arr['id'];
	    }
    } else {
 		 ...
 		 if ($this->_need_wakeup == true)
 		 {
  		 ...
  		 foreach ($this->dbg2_data as $key => $value)
	 		 ...
	 		 $this->dbg1_res =  $this->dbg3_parent->query("SELECT ".$this->db5_fields." FROM ".$this->dbg4_table." WHERE $tblkey.$idkey IN (".implode(', ',$this->ids).")");	
 		 	 ...
 		 	 mysqli_data_seek($this->dbg1_res, $this->start);
  ...
  {
  	if (!mysqli_data_seek($this->dbg1_res, $offset))
  	{
  	...
  	$this->start = $offset+1;
  	mysqli_data_seek($this->dbg1_res, $offset);
  }
     ...
     if (is_resource($this->dbg1_res))
     {
 		    ...
 		    @mysqli_free_result($this->dbg1_res);
		 }		
 	}
  ...
  function duplicate()
  {
 	...
 	function save()
 	{
 		...
 		if (!is_array($this->dbg2_data))
 		{
 			file_put_contents('/tmp/mysql1.err', @file_get_contents('/tmp/mysql1.err').$query."\r\n");
 		}
 ...
 class cls_db_mgr_i {
  ...
  function autoreleasepool()
  {
  	 ...
  	 foreach ($this->_pool as $resid => $value)
  	 {
  	 	   ...
  	 	   {
  	 	   	  mysqli_free_result($this->_pool[$resid][0]);
  	 	   } else {
  	 	     $new[$resid] = $value;
  	 	   } 
  ...
  {
  	$properties = mysqli_fetch_field_direct($res, $i);
    return is_object($properties) ? $properties->name : NULL;
  //return mysqli_field_name ($res, $i);
  }
  
	function get_old_array($res)
	{
		$ret = new db_handle_ret_i();
 ...
 class cls_db_mysqli extends cls_db_mgr_i {
    ...
    function table_name($res, $id)
    {
    	if ($res)
    	{
    	 ...
    	 $property = mysqli_fetch_field_direct($res, $id);
  ...
  function close()
  {
    ...
    function check_conn()
    {
    	 if (!$this->conn || @mysqli_ping($this->conn) === FALSE)
    	 {
    ...
    function query($sql = '')
    {
    	 global $total_query_time, $LOGGING_DB_QUERY;
    ...
    {
    	 return mysqli_data_seek($res, $nr);
    }
    ...
    function connect($host, $user, $pass = '', $db = '', $newlink = false, $persistent = false, $compress = FALSE)
    {
       ...
       {
        $this->conn = @mysqli_connect('p:'.$host, $user, $pass);
       }
       else
       {
 ...
 if (defined('JYINGO_USE_MYSQLI'))
 {
  
	 if ($this)
	  $this->db = new cls_db_mysqli();
	 else
	 {
	   $env = new db_handle_ret_i();
	   $env->db = new cls_db_mysqli();	
	 }
